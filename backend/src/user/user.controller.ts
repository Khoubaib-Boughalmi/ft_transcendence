...
export class UserController {
	...
	async getProfile(@Req() req, @Param() params: ProfileDTO) {
		...
		if (!params.username) {
			const user = await this.userService.getProfileFull({
				id: req.user.id,
			});
		...
		const user = await this.userService.getProfileMini({
	...
	async updateSettings(@Req() req, @Body() body: ProfileDTO) {
		const alreadyExists = await this.userService.user({
	...
	async updateSettingsUploadAvatar(
		@Req() req,
	...
	async updateSettingsDeleteAvatar(@Req() req) {
		await this.userService.updateUser({
			where: { id: req.user.id },
			data: {
				avatar: 'https://i.ytimg.com/vi/FNXf9XkUZ0M/maxresdefault.jpg',
			},
		});
	...
	async updateSettingsUploadBanner(
		@Req() req,
		@UploadedFile() file: Express.Multer.File,
	) {
		try {
	...
	async updateSettingsDeleteBanner(@Req() req) {
		await this.userService.updateUser({
			where: { id: req.user.id },
			data: {
				banner: 'https://dslv9ilpbe7p1.cloudfront.net/6P2tJV-6qxBDs2T8q21wZg_store_banner_image.jpeg',
			},
		});
	...
	async addFriend(@Req() req, @Body() body: AddFriendDTO) {
		...
		if (!user || !friend) throw new HttpException('User not found', 404);
	...
	async acceptFriend(@Req() req, @Body() body: AddFriendDTO) {
		...
		const friend = await this.userService.user({ id: body.id });
		if (!user || !friend) throw new HttpException('User not found', 404);
		const isFriendRequest = await this.userService.isFriendRequest(
			user.id,
			friend.id,
		);
		if (!isFriendRequest) throw new HttpException('No friend request', 400);
	...
	async userSearch(@Req() req, @Param() params: UserSearchDTO) {
		...
		users.sort((a, b) => {
			return (
				...
				b.username.indexOf(params.username)
			);
		});
